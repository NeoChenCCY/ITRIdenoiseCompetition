...
data_dir = Path(__file__).parent.parent / 'audio/test2wav'
data_dir_files = os.listdir(data_dir)
#data_dir_save = Path(__file__).parent.parent / 'audio/vocal'
data_dir_saveFlac = Path(__file__).parent.parent / 'audio/vocalFlac'
...
for i_name in data_dir_files:
    ...
    f_Name = "vocal_" + str(i).rjust(5,"0") + ".wav"
    refOUT_path = data_dir_saveFlac / f_Name
    # write to .wav file 
    #sf.write('out.wav', out_file, fs)
    sf.write(refOUT_path, out_file, fs)
    
    sample_rate, ref = scipy.io.wavfile.read(refOUT_path)
    
    f_Name = "vocal_" + str(i).rjust(5,"0") + ".flac"
    refOUTflac_path = data_dir_saveFlac / f_Name
    scipy.io.wavfile.write(refOUTflac_path, sample_rate, ref)
